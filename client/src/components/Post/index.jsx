		...
		<div className='post'>
			<div className='post-wrapper'>
					...
					<div className='post-top-left'>
						<Link to={"/profile/" + user.userName}>
							<img
								...
								style={{ position: "relative", left: "-250px", top: "-30px" }}
								src={"/assets/staticImages/no_pf_img.png"}
								alt=''
							/>
						</Link>
						<Link
				...
				<div className='post-center'>
					...
					<img
						className='post-img'
						src={post.img && POST_IMG + post.img}
						alt=''
					/>
				...
				<div className='post-bottom'>
					<div className='post-bottom-left'>
						<img
							className='like-icon'
							src='/assets/staticImages/like_icon.png'
							alt=''
							onClick={likeHandler}
						/>
					</div>
							...
							post.comments.map((c) => (
								<li className='comments-list-item' key={c._id + "-1"}>
									<div className='comment-left' key={c._id + "0"}>
										<span className='comments-list-text' key={c._id}>
											{c.body}
											<br />
											<small className='comment-time' key={c._id + "3"}>
												{c && format(c.createdAt)}
											</small>{" "}
										</span>
									</div>
										...
										<div className='delete-comment' key={c._id + "2"}>
											{currentUser._id === post.userId ? (
												<button
													...
													onClick={async () => (
														await axios.delete(
															`${apiUrl}/posts/${post._id}/comments/${c._id}`,
														),
														window.location.reload()
													)}>
													Delete
